...
You can embed a read-only chart on any web page with two lines of HTML.

**Step 1.** Add the embed script to your `<head>`:

```html
<script src="https://cdn.jsdelivr.net/gh/ImmersiveExperienceDesignTaxonomy/chart@v1.0.0/docs/embed.js"></script>
```

**Step 2.** Place a container element in your page body:

```html
<div data-taxonomy-chart
     data-state="..."
     data-theme="dark"
     style="width:100%;height:500px"></div>
```

The easiest way to get the `data-state` value is to use the Chart Builder's **Embed** button, which generates the snippet with your current profiles encoded.

### Data Attributes

| Attribute | Values | Default | Description |
|-----------|--------|---------|-------------|
| `data-taxonomy-chart` | *(presence)* | — | Required. Marks the element as an embed container. |
| `data-state` | Base64 string | — | Encoded profile data (generated by the Chart Builder). |
| `data-theme` | `dark`, `light` | Auto (system) | Color theme. Omit for automatic detection. |
| `data-labels` | `true`, `false` | `false` | Always show dimension category names alongside icons. |

Multiple charts can be embedded on the same page — each `[data-taxonomy-chart]` div is initialized independently.
